	...
	vector<float>& logit_weight)
{
	...
	this->decode_length = decode_length;
 ...
 */
{
	...
	{
	  vector<float>::iterator begin = weight_language.begin() + hidden_num * language_id[i];
		            ...
		            vector<float>& encdec_bias)
{
	for (int i = 0; i < layer_num; i++)
	{
		...
		LayerPreprocess(attention_out, batch_size, 1, weight[i][0].data(), weight[i][1].data());	
		SelfAttention(attention_out.data(),batch_size, 1,  weight[i][2].data(), weight[i][3].data(), weight[i][4].data(), weight[i][18].data(), weight[i][19].data(), weight[i][5].data(), attention_out.data(), cache_k[i], cache_v[i], self_bias.data());
		LayerPostprocess(embedding_word, attention_out);
		//encdec_atttion
		attention_out = embedding_word;
		LayerPreprocess(attention_out, batch_size, 1, weight[i][6].data(), weight[i][7].data());
		EncdecAttention(attention_out.data(), batch_size, 1, weight[i][8].data(), weight[i][11].data(), attention_out.data(), encode_out_k[i], encode_out_v[i], encdec_bias.data());
		LayerPostprocess(embedding_word, attention_out);
		//fnn
		vector<float> ffn_out_1(filter_size*batch_size, 0.0);
		attention_out = embedding_word;
		//vector<float> ffn_out_2(hidden_num, 0.0);
		LayerPreprocess(attention_out, batch_size, 1, weight[i][12].data(), weight[i][13].data());
		FeedForward(attention_out, ffn_out_1,batch_size, 1, filter_size, weight[i][14].data(), weight[i][15].data(), "relu");
		FeedForward(ffn_out_1, attention_out, batch_size, 1, hidden_num, weight[i][16].data(), weight[i][17].data(), "none");
		LayerPostprocess(embedding_word, attention_out);
	}
	//layer_preprocess
	LayerPreprocess(embedding_word, batch_size, 1, weight_scale.data(), weight_bias.data());
	//to logit
	vector<float> logit(batch_size * vocabe_size, 0.0);
	ToLogits(embedding_word.data(), batch_size, logit_weight.data(), logit.data());
	//get_word
	vector<size_t> word = GetMax(logit, batch_size);
	return word;
}

void NMT::Decoder::Translate(vector<float>& encode_out, const size_t& batch_size, const size_t& length, vector<int>& language_id, vector<int>& mask)
{
